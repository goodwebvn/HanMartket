<?php
// Heading
$_['heading_title']              = 'Orders';

// Text
$_['text_account']               = 'Tài khoản';
$_['text_order']                 = 'Đơn hàng #%s';
$_['text_order_id']              = 'Mã đơn hàng';
$_['text_invoice_no']            = 'Số hóa đơn';
$_['text_tbc']                   = 'TBC';
$_['text_shipping_address']      = 'Địa chỉ giao hàng';
$_['text_payment_address']       = 'Địa chỉ thanh toán';
$_['text_shipping_method']       = 'Phương thức giao hàng';
$_['text_payment_method']        = 'Phương thức thanh toán';
$_['text_model']                 = 'Mã';
$_['text_points']                = 'Điểm thưởng';
$_['text_subscription']          = 'Đăng ký';
$_['text_subscription_trial']    = '%s mỗi %d %s(s) cho %d thanh toán(s) sau đó ';
$_['text_subscription_duration'] = '%s mỗi %d %s(s) cho %d thanh toán(s)';
$_['text_subscription_cancel']   = '%s mỗi %d %s(s) cho đến khi hủy';
$_['text_day']                   = 'ngày';
$_['text_week']                  = 'tuần';
$_['text_semi_month']            = 'nửa tháng';
$_['text_month']                 = 'tháng';
$_['text_year']                  = 'năm';
$_['text_date_next']             = 'Ngày tiếp theo';
$_['text_remaining']             = 'Còn lại';
$_['text_comment']               = 'Ghi chú';
$_['text_history']               = 'Lịch sử đơn hàng';
$_['text_success']               = 'Thành công: Bạn đã thêm <a href="%s">%s</a> vào <a href="%s">giỏ hàng</a>!';
$_['text_no_results']            = 'Bạn chưa có đơn hàng nào trước đây!';

// Column
$_['column_order_id']            = 'Mã đơn hàng';
$_['column_customer']            = 'Khách hàng';
$_['column_product']             = 'Sản phẩm';
$_['column_product_total']       = 'Số lượng sản phẩm';
$_['column_status']              = 'Trạng thái';
$_['column_total']               = 'Tổng cộng';
$_['column_date_added']          = 'Ngày tạo';
$_['column_quantity']            = 'Số lượng';
$_['column_price']               = 'Giá';
$_['column_comment']             = 'Ghi chú';
$_['column_action']              = 'Hành động';

// Error
$_['error_reorder']              = 'Sản phẩm bạn đang cố gắng mua lại gặp vấn đề. Vui lòng kiểm tra các thông báo lỗi bên dưới.';
